...
- 不读写 `projects/*/agents/*/content-bank/` 目录
- 不用文件系统 symlink、review queue 文件、metrics.json 等
- 不调用任何依赖本地文件路径的 runner/script 逻辑
- API 路由里如果有 `hasMultica()` 判断，Multica 分支必须走完整的 DB 路径后 return，绝不 fall through 到文件系统分支
