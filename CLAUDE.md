...
- GTM workspace slug is hardcoded as `'GTM'` when querying agents
...
- `/api/agents` — reads agents from DB only (multica → GTM workspace 'GTM'; gtm-db → by project slug), no filesystem fallback
